...
var VoiceroVoice = {
  ...
  init: function () {
    ...
    if (
      ...
      var mostRecentThread = sortedThreads[0];
      ...
      if (
        ...
        var nonSystemMessages = mostRecentThread.messages.filter(
  ...
  createVoiceChatInterface: function () {
    ...
    var existingInterface = document.getElementById("voice-chat-interface");
    var existingMessagesContainer = document.getElementById("voice-messages");
    ...
    var rightButtonsContainer = document.createElement("div");
    ...
    var verifyMessagesContainer = document.getElementById("voice-messages");
    var verifyUserMessage = document.querySelector(".user-message");
    var verifyAiMessage = document.querySelector(".ai-message");

    // After creating all elements, set the padding again to override any potential changes
    setTimeout(() => {
      var messagesEl = document.getElementById("voice-messages");
      if (messagesEl) {
      ...
      var headerEl = document.getElementById("voice-controls-header");
      if (headerEl) {
      ...
      var inputWrapperEl = document.getElementById("voice-input-wrapper");
      if (inputWrapperEl) {
      ...
      var maximizeBtn = document.getElementById("maximize-voice-chat");
      if (maximizeBtn) {
  ...
  openVoiceChat: function () {
    ...
    var textInterface = document.getElementById("voicero-text-chat-container");
  ...
  minimizeVoiceChat: function () {
    ...
    var messagesContainer = document.getElementById("voice-messages");
    var headerContainer = document.getElementById("voice-controls-header");
    var inputWrapper = document.getElementById("voice-input-wrapper");
    var maximizeButton = document.getElementById("maximize-voice-chat");
  ...
  maximizeVoiceChat: function () {
    ...
    if (messagesContainer) {
      ...
      messagesContainer
        ...
        .forEach((msg) => {
          var textEl = msg.querySelector(".message-content");
  ...
  toggleMic: function (source = "manual") {
    ...
    if (this.isRecording) {
      ...
      var listeningIndicator = document.getElementById(
      ...
      var audioContextSupported = this.isAudioContextSupported();
          ...
          this.mediaRecorder.onstop = async () => {
            ...
            if (audioBlob.size > 0) {
              try {
                ...
                var whisperResponse = await fetch(
                ...
                if (!whisperResponse.ok) {
                  let errorText;
                  try {
                    var errorData = await whisperResponse.json();
                ...
                var whisperData = await whisperResponse.json();
                ...
                var cleanedTranscription = transcription.trim();
                var wordCount = cleanedTranscription.split(/\s+/).length;

                // Common patterns of random noise transcriptions
                var appearsToBeForeignOrNonsense = (text) => {
                ...
                var transEl = document.getElementById(
                ...
                if (!chatResponse.ok) {
                  let errorText;
                  try {
                    var errorData = await chatResponse.json();
                    errorText = JSON.stringify(errorData);
                  } catch (e) {
                    // If JSON parsing fails, get text response instead
                ...
                try {
                  ...
                  // Then try to parse the response as JSON if it's a string
                    ...
                    try {
                      var parsedResponse = JSON.parse(chatData.response);
                      aiTextResponse =
                ...
                var cleanedTextResponse = processedResponse.text;
                var extractedUrls = processedResponse.urls;

                try {
                  // Request audio generation using TTS endpoint through WordPress proxy
                  var ttsResponse = await fetch("/wp-json/voicero/v1/tts", {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                      ...(window.voiceroConfig?.getAuthHeaders
                  ...
                  var listenEl = document.getElementById(
                    "listening-indicator-message"
                  );
                  ...
                  if (contentType && contentType.includes("audio/")) {
                    ...
                    var responseData = await ttsResponse.json();
                    var { success, url } = responseData;
                  ...
                  var audioPlaybackPromise = new Promise((resolve) => {
                ...
                var existingSuccessMessage = document.querySelector(
                ...
                if (!existingSuccessMessage) {
                  ...
                  var messagesContainer =
                    document.getElementById("voice-messages");
                  if (messagesContainer) {
                    var placeholders = messagesContainer.querySelectorAll(
                  ...
                  var aiMessageDiv = document.querySelector(
  ...
  playAudioResponse: async function (audioBlob) {
    return new Promise((resolve, reject) => {
      try {
        ...
        var properBlob = new Blob([audioBlob], {
        ...
        function tryFallbackMethod() {
          ...
          // Create a blob URL
          var audioUrl = URL.createObjectURL(properBlob);
        ...
        function tryMP3Fallback() {
          ...
          function tryFormat(mimeType) {
            ...
            var formatBlob = new Blob([audioBlob], { type: mimeType });
            var formatAudio = new Audio();
            var formatUrl = URL.createObjectURL(formatBlob);
  ...
  playWithWebAudio: async function (audioBlob, resolve) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        var firstBytes = byteView.slice(0, 16);
        ...
        context.decodeAudioData(
          ...
          (buffer) => {
            ...
            var audioDuration = buffer.duration * 1000; // Convert to milliseconds
  ...
  extractAndCleanUrls: function (text) {
    ...
    var textWithoutMarkdown = cleanedText;
  ...
  getPastContext: function () {
    ...
    if (
      ...
      var msgs = [...window.VoiceroCore.thread.messages].sort(
        (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
      );
      var userMsgs = msgs.filter((m) => m.role === "user").slice(-5);
      var aiMsgs = msgs.filter((m) => m.role === "assistant").slice(-5);
      var last = [...userMsgs, ...aiMsgs].sort(
        (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
      );
    ...
    if (
      ...
      var voiceMessages = window.VoiceroCore.appState.voiceMessages || {};
  ...
  addTypingIndicator: function () {
    ...
    var sendButton = document.getElementById("voice-send-button");
  ...
  removeTypingIndicator: function () {
    ...
    var indicator = document.getElementById("voice-typing-indicator");
    if (indicator) {
      // If indicator is inside a wrapper, remove the wrapper instead
      var wrapper = indicator.closest(".typing-wrapper");
      if (wrapper) {
  ...
  addMessage: function (content, role, formatMarkdown = false) {
    ...
    if (!messagesContainer) {
      ...
      if (!messagesContainer) {
        ...
        if (interfaceExists) {
          var interfaceElement = document.getElementById(
    ...
    if (role === "ai") {
      var lastAi = messagesContainer.querySelector(
    ...
    if (
      ...
      var existingPlaceholders = messagesContainer.querySelectorAll(
        ".ai-message.placeholder"
      );
    ...
    if (role === "ai") {
      ...
      var containsHtml = /<[a-z][\s\S]*>/i.test(content);
    ...
    if (role === "user") {
      ...
      if (
        ...
        var userStatusDivs = messagesContainer.querySelectorAll(".read-status");
  ...
  reopenVoiceChat: function () {
    ...
    var wasOpeningBefore = this.isOpeningVoiceChat;
    this.isOpeningVoiceChat = true;
    this.isClosingVoiceChat = false;
  ...
  clearChatHistory: function () {
    ...
    if (window.VoiceroCore && window.VoiceroCore.sessionId) {
      var proxyUrl = "/wp-json/voicero/v1/session_clear";
    ...
    var messages = messagesContainer.querySelectorAll(
  ...
  loadMessagesFromSession: function () {
    ...
    if (
      ...
      if (
        ...
        // Check if there are any non-system messages
        var nonSystemMessages = currentThread.messages.filter(
          (msg) => msg.role !== "system" && msg.type !== "page_data"
        );
    ...
    if (
      ...
      if (
        ...
        var existingWelcomeMessages = document.querySelectorAll(
...
`;
  ...
  collectPageData: function () {
    // First, check if orders-container exists and temporarily remove it
    var ordersContainer = document.getElementById("orders-container");
    let ordersContainerParent = null;
    let ordersContainerNextSibling = null;

    // Save the container's position before removing it
    if (ordersContainer) {
      ordersContainerParent = ordersContainer.parentNode;
      ordersContainerNextSibling = ordersContainer.nextSibling;
      ordersContainerParent.removeChild(ordersContainer);
    }
    ...
    if (
      ...
      // Restore the orders-container if it was removed
      if (ordersContainer && ordersContainerParent) {
        if (ordersContainerNextSibling) {
          ordersContainerParent.insertBefore(
            ordersContainer,
            ordersContainerNextSibling
          );
        } else {
          ordersContainerParent.appendChild(ordersContainer);
        }
      }
    ...
    var pageData = window.VoiceroPageData.collectPageData();
  ...
  setStatus: function (status) {
    var statusElement = document.getElementById("voicero-status");
    if (statusElement) {
  ...
  sendUserTextToAPI: function (text) {
    ...
    fetch("/wp-json/voicero/v1/wordpress/chat", {
      ...
      .then((data) => {
        ...
        var placeholder = document.querySelector(".ai-message.placeholder");
        if (placeholder) {
          placeholder.remove();
        }
...
VoiceroVoice.debugInterface = function () {
  ...
  // Check visibility
  if (voiceInterface) {
    var style = window.getComputedStyle(voiceInterface);
  }

  if (mainButton) {
    var style = window.getComputedStyle(mainButton);
  }
